...
/**
 * Servlet implementation class DoFindPW
 */
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			if (ckphone.equals(dbphone)) {
				...
				while(rs.next()) {
					if(ckid.equals(rs.getString("id"))) {
						
							RequestDispatcher view = request.getRequestDispatcher("checkfindPW.jsp");
							view.forward(request, response);

					}else {
						out.println("<script>");
						out.println("alert('입력하신 정보에 맞는 PW가 없습니다.');");
