...
<meta charset="UTF-8">
<title>듣는 미술관</title>
<style>a {color:#FFBF00; text-decoration: none;} 
...
<h3 class="category" style="color: #FFBF00"><ins>자연</ins></h3>
<%!
...
try {
		...
		String sqlSt = "SELECT pair.imgName, pair.songName, pair.id,(SELECT imgpath FROM imgg WHERE imgid = id) AS imgpath FROM pair JOIN imgg WHERE imgid in (SELECT id FROM test.imgg WHERE imgCate = 'natural') GROUP By imgid;";
...
<label > IMG - <%=imgName[k]%></label><br/>
<label> SONG - <%=songName[k]%></label><br/>
</h6>
<a href = "view.jsp?pairid=<%=pairid[k]%>"><img src = "<%=imgpath[k]%>" width = "250" height = "250"></img></a>
